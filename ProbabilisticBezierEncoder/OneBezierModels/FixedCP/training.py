...
def train_one_bezier_transformer(model, dataset, batch_size, num_epochs, optimizer, loss_mode,
    ...
    if debug:
        # Tensorboard writter
        writer = SummaryWriter(basedir + "/graphics/ProbabilisticBezierEncoder/OneBezierModels/FixedCP/loss"+str(loss_mode[0]+"_distance"+str(loss_mode[1])+"_penalization"+str(penalization_coef)))
        counter = 0
    ...
    for epoch in range(num_epochs):
        ...
        for i in range(0, len(im_training)-batch_size+1, batch_size):
            ...
            if loss_mode[0] == 'pmap':
                ...
                loss = pmap_loss(control_points, num_cps, actual_covariances, im, loss_im, probabilistic_map_generator, mode='d')
            ...
            for j in range(0, len(im_validation) - batch_size + 1, batch_size):
                ...
                if loss_mode[0] == 'pmap':
                    ...
                    loss = pmap_loss(control_points, num_cps, actual_covariances, im, loss_im,
                                     probabilistic_map_generator, mode='d')
                elif loss_mode[0] == 'dmap':
                    loss = dmap_loss(control_points, num_cps, im, grid, distance=loss_mode[1])
                cummulative_loss += loss
