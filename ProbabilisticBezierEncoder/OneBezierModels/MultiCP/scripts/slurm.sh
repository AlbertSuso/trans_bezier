...
#SBATCH --output=./ProbabilisticBezierEncoder/OneBezierModels/MultiCP/transformer_bezier.out
#################
# a file for errors from the job
#SBATCH --error=./ProbabilisticBezierEncoder/OneBezierModels/MultiCP/transformer_bezier.err
#################
...
. ProbabilisticBezierEncoder/OneBezierModels/MultiCP/scripts/experiments.sh

#################
# Prepare the experiment to run
CODE="python -u main.py --num_experiment $1 --new_model $new_model --num_transformer_layers $num_transformer_layers --num_control_points $num_control_points --cp_variance $cp_variance --variance_drop $variance_drop --epochs_drop $epochs_drop --min_variance $min_variance --penalization_coef $penalization_coef --batch_size $batch_size --num_epochs $num_epochs --learning_rate $learning_rate"
