...
#SBATCH --output=./ProbabilisticBezierEncoder/MultiBezierModels/ParallelVersion/transformer_bezier.out
#################
...
. ProbabilisticBezierEncoder/MultiBezierModels/ParallelVersion/scripts/experiments.sh

#################
# Prepare the experiment to run
CODE="python -u main.py --num_experiment $1 --new_model $new_model --num_transformer_layers $num_transformer_layers --num_control_points $num_control_points --max_beziers $max_beziers --batch_size $batch_size --num_epochs $num_epochs --learning_rate $learning_rate --curv_pen_coef $curv_pen_coef"
