...
def bezier(CP, num_cps, t, device='cuda'):
    ...
    output[torch.sum(output, dim=(1, 2)) == 0] = 500*torch.ones((t.shape[1], 2), device=output.device)
    return output
...
if __name__ == '__main__':
    ...
    for num_cp in [3]:
        ...
        im = generate1bezier(im_size=im_size, batch_size=10000, max_beziers=max_beziers, num_control_points=num_cp, device='cuda')
        im = im.to('cpu')
        """seq = seq.to('cpu')
        tgt_padding_mask = tgt_padding_mask.to('cpu')"""
